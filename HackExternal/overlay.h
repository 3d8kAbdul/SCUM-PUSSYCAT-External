...
auto HealthBar(float x, float y, float w, float h, int phealth, bool Outlined) -> VOID
{
	...
	int healthValue = max(0, min(phealth, 160)); // Изменили верхний предел на 133

	ImColor barColor = ImColor
	(
		min(255 * (160 - healthValue) / 160, 255), min(255 * healthValue / 100, 255), // Изменили верхний предел на 133
		25,
		255
	);
	if (Outlined)
	...
	RectFilled(x, y, x + w, y + (int)(((float)h / 160.0f) * (float)phealth), barColor, 0.0f, 0); // Изменили верхний предел на 133
}
